...
void flip(uint32_t reset)
{
  ...
  if (reset==0)//when AUX1 is on
  {
    switch (state)
    {
       ...
       /* case VERTICAL ://when drone is near 90 degrees
            if (inclination.values.pitchDeciDegrees <= -700 && inclination.values.rollDeciDegrees >= -250 && inclination.values.rollDeciDegrees <= 250 )
            {
                //if (IS_RC_MODE_ACTIVE(BOXBARO)==1){
                DEACTIVATE_RC_MODE(BOXBARO);
                //}

                //if (IS_RC_MODE_ACTIVE(ANGLE_MODE)==1){
                DISABLE_FLIGHT_MODE(ANGLE_MODE);
                //}
                led1_op(true);
                led0_op(false);
        ...
        case EXITFLIP ://State=3//when drone has almost completed the flip and is near to 0 degrees
